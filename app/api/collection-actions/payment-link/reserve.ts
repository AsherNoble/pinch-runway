...
// A reservation lease: a `reserving` row older than this (a crash between reserve and the Pinch response) is reclaimable, so a stuck reservation cannot block the invoice for the rest of the Sydney day.
...
/**
 * The payment-link reservation/creation/reuse flow, minus the operator auth
 * guard (which stays in the route). Extracted so the integration suite can
 * drive it against a real D1 binding without a ChatGPT request context.
 */
export async function reservePaymentLink(request: Request): Promise<Response> {
  ...
  try {
    const snapshot = await loadLivePinchSnapshot(day);
